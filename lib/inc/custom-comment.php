...
function la_crida_comment_form($fields) {
...
function la_crida_comment_field($comment_field) {
...
function la_crida_custom_comment( $comment, $args, $depth ) {
	...
	<li class="post pingback">
		<p><?php _e( 'Incoming link:', 'la_crida' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __( '(Edit)', 'la_crida' ), ' ' ); ?></p>
	<?php
			break;
		default :
	?>
		...
		<div id="comment-<?php comment_ID(); ?>" class="comment">
			<header><?php echo get_avatar( $comment, 40 ); ?>
				<div class="comment-author vcard">
					...
					<?php printf( __( '%s <span class="says">says:</span>', 'la_crida' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?></h5>
				<span class="comment-meta commentmetadata">
					<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>"><time pubdate datetime="<?php comment_time( 'c' ); ?>">
					<?php
						/* translators: 1: date, 2: time */
						printf( __( '%1$s at %2$s', 'la_crida' ), get_comment_date(), get_comment_time() ); ?>
					</time></a>
					<?php edit_comment_link( __( '(Edit)', 'la_crida' ), ' ' );
					?>
				</span><!-- .comment-meta .commentmetadata -->
				</div><!-- .comment-author .vcard -->
				<?php if ( $comment->comment_approved == '0' ) : ?>
					<em><?php _e( 'Your comment is awaiting moderation.', 'la_crida' ); ?></em>
					<br />
				<?php endif; ?>
